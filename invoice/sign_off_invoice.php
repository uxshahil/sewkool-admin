...
<form name="sign_off_invoice" action="sign_off_invoice_function.php?action=sign_off_invoice" method="post">
    <table class='table table-hover table-responsive table-bordered box'>
        ...
        <tr>
			<td>Signed Off - By</td>
			<td><input type='text' name='signed_off_by_name' <?php echo "value='{$_SESSION['first_name']}'" ?> readonly='readonly' class='form-control' /></td>
            <td><input type='hidden' name='signed_off_by' <?php echo "value='{$_SESSION['user_id']}'" ?> readonly='readonly' class='form-control' /></td>
		</tr>

        <tr>
            <td></td>
            <td>
                <button class="btn btn-primary btn-block" onClick="submitForm()">Update</h3></button>
            </td>
        </tr>
