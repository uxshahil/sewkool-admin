...
class line_item{
    ...
    public function uploadPhoto(){
        ...
        public function updatePhoto(){
            ...
            if($this->artwork_logo){
                ...
                // make sure that file is a real image
                $check = getimagesize($_FILES["artwork_logo_new"]["tmp_name"]);
                if($check!==false){
                    // submitted file is an image
                }else{
                    $file_upload_error_messages.="<div>Submitted file is not an image</div>";
                ...
                // make sure submitted file is not too large, can't be larger than 10 MB
                if($_FILES['artwork_logo_new']['size'] > (10240000)){
                    $file_upload_error_messages.="<div>Image must be less than 10 MB in size.</div>";
                }
                ...
                if(empty($file_upload_error_messages)){
                    // it means there are no errors, so try to upload the file
                    if(move_uploaded_file($_FILES["artwork_logo_new"]["tmp_name"], $target_file)){
                        // means photo was uploaded
                    }else{
                        $result_message.="<div class='alert alert-danger'>";
                            $result_message.="<div>Unable to upload photo.</div>";
    ...
    // used to read line_item by job_card_id
    function readJobCardItems(){

        $query = "SELECT * FROM " . $this->table_name . " WHERE job_card_id = ?";
